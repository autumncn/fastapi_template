...
async def read_permission_list(request: Request, skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
...
async def create_permission_get(request: Request, db: AsyncSession = Depends(get_db)):
    new_permission = PermissionCreate
    user_list = await UserService().get_user_list()
    menu_list = await MenuService().menu_list_id_name()
...
@router.get("/{id}/view")
async def read_permission_by_id(request: Request, id: str, db: AsyncSession = Depends(get_db)):
    ...
    user_name_list = await UserService().get_user_name_by_id_list(user_ids=user_ids)
    new_permission['user_name_list'] = user_name_list

    menu_ids = new_permission['menu_ids'].split(", ")
    menu_name_list = await MenuService().get_menu_name_by_id_list(menu_ids=menu_ids)
...
async def edit_permission_by_id_get(request: Request, id: str, db: AsyncSession = Depends(get_db)):
    permission = get_permission(db, permission_id=int(id))
    if permission is None:
        request.session["error"] = _("permission_not_found")
        return RedirectResponse('/permissions', status_code=status.HTTP_404_NOT_FOUND)
...
async def delete_permission(request: Request, id: str, db: AsyncSession = Depends(get_db)
):
