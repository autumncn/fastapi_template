...
async def get_sd_tasks(db: AsyncSession, skip: int = 0, limit: int = 100):
    return db.query(SdTask).offset(skip).limit(limit).all()

async def get_sd_tasks_by_user(db: AsyncSession, user: str, skip: int = 0, limit: int = 100):
    return db.query(SdTask).filter(SdTask.create_by == user).offset(skip).limit(limit).all()

async def get_sd_tasks_by_user_type(db: AsyncSession, user: str, type: str, skip: int = 0, limit: int = 100):
    return db.query(SdTask).filter(SdTask.create_by == user and SdTask.type == type).offset(skip).limit(limit).all()


async def get_sd_task(db: AsyncSession, sdTask_id: int):
    return db.query(SdTask).filter(SdTask.id == sdTask_id).first()


async def get_sd_task_by_uuid(db: AsyncSession, sdTask_uid: str):
    return db.query(SdTask).filter(SdTask.uid == sdTask_uid).first()

async def create_sdTask(db: AsyncSession, sdTask: SdTaskCreate):
...
async def modify_sdTask(db: AsyncSession, sdTask: SdTaskModify):
...
async def delete_sd_task_by_id(db: AsyncSession, id: int):
    db_task = get_sd_task(db, id)
    db.delete(db_task)
    db.commit()

async def delete_sd_task_by_uid(db: AsyncSession, uid: str):
