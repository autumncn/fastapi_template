...
def get_menu_list(skip: int = 0, limit: int = 100, db=next(get_db())):
...
def get_menu_list_by_user_permission(user_login_email: str, skip: int = 0, limit: int = 100, db=next(get_db())):
...
def get_menu_path_by_user_permission(user_login_email: str, db=next(get_db())):
    login_user = get_user_by_email(user_login_email, db)
    user_role_id = login_user.user_role_id
    user_permission = get_permission(db, user_role_id)
    user_menu_ids = (user_permission.menu_ids).split(',')
...
def find_parent_menu(menu_path: str, db=next(get_db())):
    menu_dict = {}
...
def menu_list_id_name(skip: int = 0, limit: int = 100, db=next(get_db())):
    menus = get_menus(db, skip=skip, limit=limit)
    menu_list = []
    for menu in menus:
        menu_dict = {
...
def get_menu_name_by_id_list(menu_ids=List[int], db=next(get_db())):
